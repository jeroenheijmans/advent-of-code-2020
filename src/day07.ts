...
let data = input
  ...
  .map(line => {
    ...
    return {
      source: parts[0].replace(/bags?/, "").trim(),
      targets: parts[1].startsWith("no ") ? [] : parts[1]
      .slice(0, -1) // period
      .split(", ")
      .map(t => ({
        nr: parseInt(t.substr(0, 1)),
        bagtype: t.substr(2, t.length-1).replace(/ bags?/, "").trim(),
      })),
    };
  })
  ;
...
let newOuters = new Set<string>();

while(true) {
  newOuters = new Set(outers);
  outers.forEach(b => {
    data.forEach(x => {
      if (x.targets.some(y => y.bagtype === b)) {
        newOuters.add(x.source);
      }
    })
  });
  if (newOuters.size === outers.size) { break; }
  outers = newOuters;
}
